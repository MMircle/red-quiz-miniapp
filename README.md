...
随手刷题是一款红色主题刷题应用，覆盖党的基础知识、伟大建党精神、“四史”学习和中大红色校史等内容。项目同时提供 H5 版本和微信小程序版本，适合碎片化学习、课前热身、活动自测和错题复盘。
...
[随手刷题 H5](https://mmircle.github.io/red-quiz-miniapp/)

使用方式：

1. 打开链接后选择“十题快练”或“五十题精练”。
2. 单选题提交后可立即查看答案解析；多选题需要全部选对才得分。
3. 五十题精练可标记不确定题，交卷后查看错题、薄弱分类和复习建议。

H5 是纯静态页面，不需要登录，也不依赖后端。答题进度会临时保存在当前浏览器中，刷新页面后可继续作答。

### 微信小程序

扫码体验微信小程序：<br>
<img src="./logo.jpg" width="200" alt="小程序二维码">

## 功能亮点

- 十题快练：随机抽取 10 道单选题，每题提交后即时反馈。
- 五十题精练：随机抽取 40 道单选题和 10 道多选题，交卷后统一分析。
- 单选/多选：多选题支持反复点选和取消，需全部选对才得分。
- 题号面板：快速跳题，区分当前题、已作答题和已标记题。
- 标记本题：适合先标记不确定题，交卷前集中检查。
...
进行中的答题状态写入浏览器 `localStorage`，不接后端、不做登录、不做运营统计。
...
- `category` 必须在 `categoryLabels` 中配置展示名称。
...
如果本机 `node` 不可用，可使用 Codex 工作区运行时：

```bash
C:\Users\Admin\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe -e "const q=require('./utils/quiz'); console.log(q.validateQuestionBank())"
```

校验通过时，`ok` 应为 `true`，`errors` 应为空数组。

### H5 题库同步

H5 当前使用独立的 [`h5/question-bank.js`](h5/question-bank.js) 静态题库文件。更新 `data/questions/*.js` 后，需要同步更新 H5 题库文件，并修改 [`h5/index.html`](h5/index.html) 中的题库版本号，例如：

```html
<script defer src="./question-bank.js?v=20260519-3"></script>
```
...
- H5 静态题库中的中文显示标签建议使用 Unicode 转义，避免在不同本机编码环境下变成问号。
